...
| 0x04   | 4    | `checksum`               | CRC32-C checksum of the superblock block; see {ref}`superblock-checksum` |
| 0x08   | 4    | `feature_compat`         | Compatible feature flags; see {ref}`feature-flags` |
| 0x0C   | 1    | `blkszbits`              | Block size = `2^blkszbits`; minimum 9 |
| 0x0D   | 1    | `sb_extslots`            | Number of 16-byte superblock extension slots |
| 0x0E   | 2    | `rootnid_2b`             | Root directory NID (16-bit); see {ref}`root-nid-encoding` |
| 0x0E   | 2    | `blocks_hi`              | High 16 bits of total block count; see {ref}`block-count-encoding` |
...
| 0x24   | 4    | `blocks_lo`              | Low 32 bits of total block count; see {ref}`block-count-encoding` |
...
| 0x50   | 4    | `feature_incompat`       | Incompatible feature flags; see {ref}`feature-flags` |
| 0x54   | 2    | `available_compr_algs`   | Bitmap of algorithms that have per-algorithm config records in the metadata area (`EROFS_FEATURE_INCOMPAT_COMPR_CFGS` set); see {ref}`compression-configuration` |
| 0x54   | 2    | `lz4_max_distance`       | Customised LZ4 sliding-window size; 0 = default; see {ref}`compression-configuration` |
...
| 0x60   | 8    | `packed_nid`             | NID of the packed inode for fragment storage; see {ref}`packed-inode-packed_nid` |
...
| 0x70   | 8    | `rootnid_8b`             | Root directory NID (64-bit); see {ref}`root-nid-encoding` |
| 0x78   | 8    | `reserved2`              | Reserved; must be 0 |
| 0x80   | 8    | `metabox_nid`            | NID of the metabox inode; see {ref}`metabox-inode-metabox_nid` |
...
| `0x00000002` | `EROFS_FEATURE_INCOMPAT_COMPR_CFGS`     | Compression configuration is present; see {ref}`compression-configuration` |
...
| `0x00000020` | `EROFS_FEATURE_INCOMPAT_FRAGMENTS`      | Fragment packing via packed inode; see {ref}`packed-inode-packed_nid` |
...
| `0x00000080` | `EROFS_FEATURE_INCOMPAT_48BIT`          | Block addresses are extended from 32-bit to 48-bit; see {ref}`root-nid-encoding` |
| `0x00000100` | `EROFS_FEATURE_INCOMPAT_METABOX`        | Metabox inode for inline inode metadata; see {ref}`metabox-inode-metabox_nid` |
...
- `packed_nid`: NID of the packed inode. Valid when `EROFS_FEATURE_INCOMPAT_FRAGMENTS`
  is set. See {ref}`packed-inode-packed_nid`.
- `metabox_nid`: NID of the metabox inode. Valid when `EROFS_FEATURE_INCOMPAT_METABOX`
  is set. See {ref}`metabox-inode-metabox_nid`.
...
| Offset | Size | Name        | Description |
|--------|------|-------------|-------------|
| 0x00   | 8    | `nid`       | Node number of the target inode; bit 63 is the metabox flag (see {ref}`metabox-inode-metabox_nid`) |
